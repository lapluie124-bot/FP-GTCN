...
#         return edge_index
#
#     def create_gcn_lstm_data(self, window_size=10, step_size=1,
#                              target_offset=1, test_size=0.2,
...
#
#         print(f"创建训练样本 {len(train_data)} 条，测试样本 {len(test_data)} 条")
#         return train_data, test_data, edge_index, self.scaler, self.target_min, self.target_max
#
#     def save_preprocessed_data(self, train_dataset, test_dataset, edge_index, scaler, output_dir, target_min, target_max):
#         os.makedirs(output_dir, exist_ok=True)
#         # torch.save(train_dataset, f"{output_dir}/train_dataset.pt")
#         # torch.save(test_dataset, f"{output_dir}/test_dataset.pt")
#         # torch.save(edge_index, f"{output_dir}/edge_index.pt")
#         # torch.save(scaler, f"{output_dir}/scaler.pkl")
#         # torch.save({'min': target_min, 'max': target_max}, f"{output_dir}/target_range.pt")
#         if self.processed_data is not None:
#             self.processed_data.to_csv(f"{output_dir}/processed_features.csv", index=False)
#         print(f"预处理数据已保存至 {output_dir}")
#
#
# # ========== 主程序 ==========
...
#         time_col=TIME_COL
#     )
#     train_data, test_data, edge_index, scaler, target_min, target_max = processor.create_gcn_lstm_data(
...
#     )
#     processor.save_preprocessed_data(train_data, test_data, edge_index, scaler, OUTPUT_DIR, target_min, target_max)
#     print("预处理完成")
...
#     ("釜温", "中温"),
#     ("中温", "顶温"),("中温", "回流比"),
#     ("顶温", "回流比"),("顶温", "V02121A液位/cm"), ("顶温", "V02122A液位/cm"), ("顶温", "V02123A液位/cm"), ("顶温", "V02123B液位/cm"),
#     ("蒸汽开度/%", "釜温"), ("蒸汽开度/%", "回流比"),
#     ("回流比", "V02121A液位/cm"), ("回流比", "V02122A液位/cm"), ("回流比", "V02123A液位/cm"), ("回流比", "V02123B液位/cm"),
#
# ]
...
class ChemicalDataProcessor:
    ...
    def preprocess_data(self, fill_na=0.0):
        ...
        self.raw_data = df.copy()  # ✅ <== 添加这行，确保包含伪时间列
        self.target_series = df[self.target_node]
        feature_cols = [col for col in self.node_features + ['time_delta_hours', 'time_diff_hours']
                        if col in df.columns and col != self.target_node]
        self.processed_data = df[feature_cols]
